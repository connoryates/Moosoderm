...
use Data::Dumper;
use Moosoderm::Component;
use Moosoderm::Relationship;
use Moosoderm::Mapping;
...
sub build_relationship {
  ...
  my $r = Moosoderm::Relationship->new(
    name            => $c->name,
    table           => $table,
    columns         => \@columns,
...
sub write_table {
  ...
  my @rs_comp =
    ...
    map { Moosoderm::Component->find($_) }
...
sub write_preamble {
  ...
  local ($Moosoderm::VERSION)    # hide from Module::Metadata
    = (0) unless defined $Moosoderm::VERSION;
